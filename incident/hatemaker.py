...
class KantGenerator:
    ...
    def output(self):
        ...
        for n in range(0,len(self.pieces)):
        	try:
        		if len(self.pieces[n]) < 5: continue
        	except IndexError:
        		continue
        	list1 = self.pieces[:]
        	list1.pop(n)
        	for m in list1:
        		if len(m) < 5: continue
        		if m[:4] == self.pieces[n][:4]:
        			if _debug:
        				sys.stderr.write('\nerror, "%s" matches "%s" in "%s"\n\n' % (m, self.pieces[n], "".join(self.pieces)))
        			self.refresh()
        			break
...
def incident_text():
  ...
  k1 = k.refresh()
  return k1   
    #os.system("""python tweetyPy -t 'update' -u newmediafart@gmail.com -p fart4321 -s "%s" -d """ % k.output())
